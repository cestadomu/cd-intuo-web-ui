...
Ext.define('CestaDomu.controller.WorkReportsController', {
    ...
    initData: function(worksheet, employee, interval, dateFrom, dateTo, styles, buffer) {
        ...
        if (timeFundStore.getCount() != 1) {
            Ext.Msg.alert('Chyba', 'Nepodařilo se načíst časový fond zaměstnance, generování výkazu práce bylo přerušeno.');
            throw 'No time fund.';
        }
        ...
        data.push(['Práce ve státní svátek:', '', '', {value: buffer.celkem.praceSvatek/60, metadata: statisticsMetadata}]);
        worksheet.mergeCells('A'+data.length, 'C'+data.length);
        ...
        data.push(['Práce v pohotovosti bez sv.:', '', '', {value: buffer.celkem.pohotovostBezSvatku/60, metadata: statisticsMetadata}]);
        worksheet.mergeCells('A'+data.length, 'C'+data.length);
    ...
    flushBuffer: function(buffer, data, styles) {
        ...
        for (var row in dny) {
            ...
            var praceCelkem = (dny[row].nepohotovost+dny[row].pohotovost)/60;
            ...
            var dovolena =  {value: dny[row].dovolena ? dny[row].dovolena : '', metadata: workMetadata};
            data.push([
                ...
                {value: dny[row].nepohotovost ? dny[row].nepohotovost/60 : '', metadata: workMetadata},
                {value: dny[row].pohotovost ? dny[row].pohotovost/60 : '', metadata: workMetadata},
                {value: praceCelkem ? praceCelkem : '', metadata: workMetadata},
                {value: dny[row].cekaniNaPraci ? dny[row].cekaniNaPraci/60 : '', metadata: workMetadata},
                {value: dny[row].neplacene ? dny[row].neplacene/60 : '', metadata: workMetadata},
                {value: dny[row].lekar ? dny[row].lekar : '', metadata: workMetadata},
                {value: dny[row].dovolena ? dny[row].dovolena : '', metadata: workMetadata},
                {value: dny[row].paragraf ? dny[row].paragraf : '', metadata: workMetadata},
                {value: dny[row].nemoc ? dny[row].nemoc : '', metadata: workMetadata},
